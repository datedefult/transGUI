...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QButtonGroup, QCheckBox, QGroupBox, QTextEdit, QProgressBar, QTabWidget)
...
class TranslationThread(QThread):
    ...
    def __init__(self, params):
        ...
        self.last_request_time = 0  # 跟踪最后请求时间
        self.request_lock = asyncio.Lock()  # 防止并发冲突

    def run(self):
        try:
            asyncio.run(self._run_translation())
    ...
    async def _run_translation(self):
        ...
        total_tasks = total_rows * total_langs
        completed_tasks = 0
        ...
        async with aiohttp.ClientSession(
            ...
            for row_idx, row in df.iterrows():
                ...
                row_progress = int((row_idx / total_rows) * 100)
                ...
                for lang_idx, lang_code in enumerate(target_langs):
                    ...
                    try:
                        ...
                        self.progress_updated.emit(
                            overall_progress,
                            f"进度: {row_idx + 1}/{total_rows}行 | {lang_name} | 已完成: {overall_progress}%"
                        )
                        ...
                        self.progress_updated.emit(
                            ...
                            f"错误: 行{row_idx + 1} {TARGET_LANGUAGES[lang_code]}: {str(e)[:100]}"
                        )
    ...
    async def _call_translation_api(self, session, text, source_lang, target_lang):
        """调用翻译API"""
        async with self.request_lock:  # 确保线程安全
            # 计算需要等待的时间
...
class ImageViewer(QWidget):
    def __init__(self, parent=None):
        ...
        self.image_folder = r"C:\Users\Windows10\Pictures\1.0"
        self.image_files = []
    ...
    def init_ui(self):
        ...
        self.auto_btn = QPushButton("开始自动播放")
...
class TranslationApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setup_translation_ui(translation_tab)

        # 创建图片浏览器标签页
        image_viewer_tab = ImageViewer()

        # 添加标签页
        self.tab_widget.addTab(translation_tab, "文档翻译")
        self.tab_widget.addTab(image_viewer_tab, "图片浏览器")

        self.setCentralWidget(self.tab_widget)

    def setup_translation_ui(self, tab):
        """原有的翻译界面设置代码"""
        layout = QVBoxLayout()
