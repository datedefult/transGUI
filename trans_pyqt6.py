...
class APIConfig:
    ...
    def get_resource_path(cls, relative_path):
        ...
        search_paths = [
            ...
            Path(base_path) / "_internal" / relative_path
        ]
...
class TranslationThread(QThread):
    ...
    async def _run_translation(self):
        ...
        try:
            ...
            self.progress_updated.emit(5, f"成功读取文件: {os.path.basename(params['input_path'])}")
        except Exception as e:
        ...
        result_df = df.copy()
        total_tasks = len(df) * len(params['target_langs'])
        self.completed_tasks = 0
        ...
        async with aiohttp.ClientSession(
            ...
            for row_idx, row in df.iterrows():
                ...
                for lang_code in params['target_langs']:
                    task = self._create_translation_task(
                        session, result_df, row_idx, text,
                        params['source_lang'], lang_code, total_tasks
                    )
                    tasks.append(task)
        ...
        try:
            result_df.to_excel(params['output_path'], index=False)
            self.progress_updated.emit(100, f"翻译完成! 结果已保存到: {os.path.basename(params['output_path'])}")
            self.finished.emit(True, params['output_path'])
        except Exception as e:
    ...
    def _create_translation_task(self, session, result_df, row_idx, text, source_lang, lang_code, total_tasks):
        async def task():
            ...
            try:
                translated = await self._call_translation_api(session, text, source_lang, lang_code)
...
if __name__ == "__main__":
    ...
    # 设置应用图标
    icon_path = APIConfig().get_resource_path("favicon.ico")
